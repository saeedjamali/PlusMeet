...
import styles from "../../create/eventCreate.module.css";
import "../../create/eventCreateDark.css";
import Step2Details from "../../create/Step2Details";
import Step3Details from "../../create/Step3Details";
import Step4Details from "../../create/Step4Details";
import Step5Details from "../../create/Step5Details";
import Step6Details from "../../create/Step6Details";
import Step7Details from "../../create/Step7Details";
...
export default function EditEventPage() {
  ...
  const [eventData, setEventData] = useState(null); // ✅ ذخیره اطلاعات کامل رویداد
  ...
  const [formData, setFormData] = useState({
    ...
    ticket: {
      ...
      price: 0,
      discountCodes: [], // آرایه از {code: string, discountPercent: number}
      refundable: false,
      saleEndDate: "",
    },
    invitation: {
  ...
  const fetchEventData = async () => {
    try {
      ...
      // ✅ ذخیره اطلاعات کامل رویداد (برای نمایش دلیل رد و غیره)
      setEventData(event);

      // ✅ پر کردن formData با دیتای موجود
      setFormData({
        title: event.title || "",
        description: event.description || "",
        topicCategory: event.topicCategory?._id || null,
        topicSubcategory: event.topicSubcategory?._id || null,
        images: event.images || [],
        coverImage: event.coverImage || null,
        speakers: event.speakers || [],
        formatMode: event.formatMode?._id || null,
        ...
        impactPurpose: event.impactPurpose?._id || null,
        socialDynamics: event.socialDynamics?._id || null,
        audienceType: event.audienceType?._id || null,
        emotional: event.emotional?._id || null,
        intent: event.intent?._id || null,
        participationType: event.participationType?._id || null,
        tags: event.tags || [],
        ...
        eligibility: event.eligibility || ["active"],
        ...
        createGroupChat: event.createGroupChat || false,
      ...
      // ✅ تنظیم selectedCategory و selectedSubcategory
      if (event.topicCategory) {
        setSelectedCategory(event.topicCategory);
      }
      if (event.topicSubcategory) {
        setSelectedSubcategory(event.topicSubcategory);
      }
      if (event.formatMode) {
        setSelectedFormatMode(event.formatMode);
      }
      if (event.participationType) {
        setSelectedParticipationType(event.participationType);
      }
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      console.log("💾 Updating draft with currentStep:", currentStep, eventData);

      // ✅ استفاده از PUT برای ویرایش
      ...
      alert("✅ پیش‌نویس با موفقیت بروزرسانی شد!");
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      ...
      const eventData = {
        ...
        formatMode: selectedFormatMode?._id,
        participationType: selectedParticipationType?._id,
        status: "pending", // وضعیت: در انتظار تایید (برای ویرایش)
      };
      ...
      if (!response.ok) {
        throw new Error(data.error || "خطا در بروزرسانی رویداد");
      }

      console.log("✅ Event updated successfully:", data);

      // نمایش پیام موفقیت و انتقال به صفحه رویدادهای من
      alert("✅ رویداد با موفقیت بروزرسانی شد و در انتظار تایید است");
      router.push("/dashboard/myEvents");
    } catch (err) {
  ...
  return (
    <div className={styles.container}>
      ...
      {/* نمایش دلیل رد (اگر رویداد رد شده باشد) */}
      {eventData?.status === "rejected" && eventData?.rejectionReason && (
        <div className={styles.rejectionAlert}>
          <div className={styles.rejectionAlertIcon}>⚠️</div>
          <div className={styles.rejectionAlertContent}>
            <h3 className={styles.rejectionAlertTitle}>
              رویداد شما رد شده است
            </h3>
            <p className={styles.rejectionAlertReason}>
              <strong>دلیل رد:</strong> {eventData.rejectionReason}
            </p>
            <p className={styles.rejectionAlertHint}>
              لطفاً ایرادات ذکر شده را برطرف کرده و پس از ذخیره، رویداد خود را
              مجدداً ارسال نمایید.
            </p>
          </div>
        </div>
