...
export default function ParticipationTypeModal({ mode, category, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = mode === 'edit' 
        ? `/api/dashboard/participationType/${category._id}`
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2>{mode === 'edit' ? 'ویرایش نوع شرکت' : 'نوع شرکت جدید'}</h2>
