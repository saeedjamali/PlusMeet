...
import ParticipationTypeModal from './ParticipationTypeModal';

export default function ParticipationTypePage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/dashboard/participationType?${params.toString()}`, {
        credentials: 'include',
      });
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      const response = await fetch(`/api/dashboard/participationType/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
    ...
    <div className={styles.container}>
      ...
      {error ? (
        ...
        <div className={styles.empty}>
          ...
          <h3>هیچ نوع شرکت یافت نشد</h3>
          <p>برای شروع، یک نوع شرکت جدید ایجاد کنید</p>
          <button className={styles.btnPrimary} onClick={handleCreate}>
            ایجاد اولین نوع شرکت
          </button>
        </div>
      ) : (
