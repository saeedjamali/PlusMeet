...
import ImpactPurposeModal from './ImpactPurposeModal';

export default function ImpactPurposePage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/dashboard/impactPurpose?${params.toString()}`, {
        credentials: 'include',
      });
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      const response = await fetch(`/api/dashboard/impactPurpose/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
