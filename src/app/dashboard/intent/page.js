...
export default function IntentPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/dashboard/intent?${params.toString()}`, {
        credentials: 'include',
      });
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      const response = await fetch(`/api/dashboard/intent/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
    ...
    <div className={styles.container}>
      ...
      {error ? (
        ...
        <div className={styles.empty}>
          ...
          <h3>هیچ نوع تعامل یا هدف یافت نشد</h3>
          <p>برای شروع، یک نوع تعامل جدید ایجاد کنید</p>
          <button className={styles.btnPrimary} onClick={handleCreate}>
            ایجاد اولین نوع تعامل
          </button>
        </div>
      ) : (
