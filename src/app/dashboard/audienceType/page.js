...
export default function AudienceTypePage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/dashboard/audienceType?${params.toString()}`, {
        credentials: 'include',
      });
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      const response = await fetch(`/api/dashboard/audienceType/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
