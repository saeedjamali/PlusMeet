...
import styles from '../shared/categoryModal.module.css';
...
export default function SocialDynamicsModal({ mode, category, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = mode === 'edit' 
        ? `/api/dashboard/socialDynamics/${category._id}`
        : '/api/dashboard/socialDynamics';
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2>{mode === 'edit' ? 'ویرایش دسته‌بندی' : 'دسته‌بندی جدید'}</h2>
