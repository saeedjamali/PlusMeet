...
import SocialDynamicsModal from './SocialDynamicsModal';
...
export default function SocialDynamicsPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/dashboard/socialDynamics?${params.toString()}`, {
        credentials: 'include',
      });
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      const response = await fetch(`/api/dashboard/socialDynamics/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      ...
      <div className={styles.header}>
        <div className={styles.headerTop}>
          <h1 className={styles.title}>
            ...
            مدیریت دسته‌بندی تعامل اجتماعی
          </h1>
