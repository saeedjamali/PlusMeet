...
import styles from './format_mode.module.css';
import FormatModeModal from './FormatModeModal';

export default function FormatModePage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/dashboard/format_mode?${params.toString()}`, {
        credentials: 'include',
      });
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      const response = await fetch(`/api/dashboard/format_mode/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
    ...
    <div className={styles.container}>
      ...
      {error ? (
        ...
        <div className={styles.empty}>
          ...
          <h3>هیچ نوع برگزاری یافت نشد</h3>
          <p>برای شروع، یک نوع برگزاری جدید ایجاد کنید</p>
          <button className={styles.btnPrimary} onClick={handleCreate}>
            ایجاد اولین نوع برگزاری
          </button>
        </div>
      ) : (
