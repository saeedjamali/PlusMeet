...
export async function GET(request) {
  try {
    ...
    const categories = await AudienceTypeCategory.find(query)
    ...
    console.error("❌ Error fetching audience type categories:", error);
    return NextResponse.json(
      {
...
export async function POST(request) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: "شما دسترسی به ایجاد دسته‌بندی مخاطب ندارید" },
        { status: 403 }
      );
    }
    ...
    const newCategory = new AudienceTypeCategory(categoryData);
    ...
    return NextResponse.json(
      {
        success: true,
        message: "دسته‌بندی مخاطب با موفقیت ایجاد شد",
        data: newCategory,
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("❌ Error creating audience type category:", error);
    ...
    return NextResponse.json(
      {
        error: "خطا در ایجاد دسته‌بندی مخاطب",
        details: error.message,
      },
      { status: 500 }
    );
