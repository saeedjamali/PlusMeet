...
import AudienceTypeCategory from '@/lib/models/AudienceTypeCategory.model';
...
export async function GET(request, { params }) {
  try {
    ...
    const category = await AudienceTypeCategory.findById(id)
    ...
    console.error('❌ Error fetching audience type category:', error);
    return NextResponse.json(
      {
        error: 'خطا در دریافت دسته‌بندی مخاطب',
...
export async function PUT(request, { params }) {
  try {
    ...
    const hasPermission = await checkPermission(
      authResult.user.id,
      'audience_type_categories',
      'update'
    );

    if (!hasPermission) {
      return NextResponse.json(
        { error: 'شما دسترسی به ویرایش دسته‌بندی مخاطب ندارید' },
        { status: 403 }
      );
    }
    ...
    if (body.code && body.code.toUpperCase().trim() !== category.code) {
      const existingCode = await AudienceTypeCategory.findOne({
    ...
    return NextResponse.json({
      success: true,
      message: 'دسته‌بندی مخاطب با موفقیت به‌روزرسانی شد',
      data: category,
    });
  } catch (error) {
    console.error('❌ Error updating audience type category:', error);
    ...
    return NextResponse.json(
      {
        error: 'خطا در به‌روزرسانی دسته‌بندی مخاطب',
        details: error.message,
      },
      { status: 500 }
    );
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json({ error: 'شما دسترسی به حذف دسته‌بندی مخاطب ندارید' }, { status: 403 });
    }
    ...
    await AudienceTypeCategory.findByIdAndDelete(id);

    return NextResponse.json({
      success: true,
      message: 'دسته‌بندی مخاطب با موفقیت حذف شد',
    });
  } catch (error) {
    console.error('❌ Error deleting audience type category:', error);
    return NextResponse.json(
      {
        error: 'خطا در حذف دسته‌بندی مخاطب',
        details: error.message,
      },
      { status: 500 }
    );
