...
// GET - لیست دسته‌بندی احساسی / هدف‌محور
export async function GET(request) {
  try {
    ...
    const categories = await EmotionalCategory.find(query)
    ...
    console.error("❌ Error fetching emotional categories:", error);
    return NextResponse.json(
      {
...
export async function POST(request) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: "شما دسترسی به ایجاد دسته‌بندی احساسی / هدف‌محور ندارید" },
        { status: 403 }
      );
    }
    ...
    const newCategory = new EmotionalCategory(categoryData);
    ...
    return NextResponse.json(
      {
        success: true,
        message: "دسته‌بندی احساسی / هدف‌محور با موفقیت ایجاد شد",
        data: newCategory,
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("❌ Error creating emotional category:", error);
    ...
    return NextResponse.json(
      {
        error: "خطا در ایجاد دسته‌بندی احساسی / هدف‌محور",
        details: error.message,
      },
      { status: 500 }
    );
