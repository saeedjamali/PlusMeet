...
import EmotionalCategory from '@/lib/models/EmotionalCategory.model';
...
export async function GET(request, { params }) {
  try {
    ...
    console.error('❌ Error fetching emotional category:', error);
    return NextResponse.json(
      {
        error: 'خطا در دریافت دسته‌بندی احساسی / هدف‌محور',
...
export async function PUT(request, { params }) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: 'شما دسترسی به ویرایش دسته‌بندی احساسی / هدف‌محور ندارید' },
        { status: 403 }
      );
    }
    ...
    if (body.code && body.code.toUpperCase().trim() !== category.code) {
      const existingCode = await EmotionalCategory.findOne({
    ...
    return NextResponse.json({
      success: true,
      message: 'دسته‌بندی احساسی / هدف‌محور با موفقیت به‌روزرسانی شد',
      data: category,
    });
  } catch (error) {
    console.error('❌ Error updating emotional category:', error);
    ...
    return NextResponse.json(
      {
        error: 'خطا در به‌روزرسانی دسته‌بندی احساسی / هدف‌محور',
        details: error.message,
      },
      { status: 500 }
    );
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json({ error: 'شما دسترسی به حذف دسته‌بندی احساسی / هدف‌محور ندارید' }, { status: 403 });
    }
    ...
    await EmotionalCategory.findByIdAndDelete(id);

    return NextResponse.json({
      success: true,
      message: 'دسته‌بندی احساسی / هدف‌محور با موفقیت حذف شد',
    });
  } catch (error) {
    console.error('❌ Error deleting emotional category:', error);
    return NextResponse.json(
      {
        error: 'خطا در حذف دسته‌بندی احساسی / هدف‌محور',
        details: error.message,
      },
      { status: 500 }
    );
