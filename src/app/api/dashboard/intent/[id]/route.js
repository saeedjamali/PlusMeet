...
export async function GET(request, { params }) {
  try {
    ...
    const category = await IntentCategory.findById(id)
      .populate('createdBy', 'firstName lastName email phoneNumber')
      .populate('updatedBy', 'firstName lastName email phoneNumber');

    if (!category) {
      return NextResponse.json({ error: 'نوع تعامل یا هدف یافت نشد' }, { status: 404 });
    }
    ...
    console.error('❌ Error fetching intent category:', error);
    return NextResponse.json(
      {
...
export async function PUT(request, { params }) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: 'شما دسترسی به ویرایش نوع تعامل یا هدف ندارید' },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'نوع تعامل یا هدف با موفقیت به‌روزرسانی شد',
      data: category,
    });
  } catch (error) {
    console.error('❌ Error updating intent category:', error);
    ...
    return NextResponse.json(
      {
        error: 'خطا در به‌روزرسانی نوع تعامل یا هدف',
        details: error.message,
      },
      { status: 500 }
    );
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json({ error: 'شما دسترسی به حذف نوع تعامل یا هدف ندارید' }, { status: 403 });
    }
    ...
    if (category.eventsCount > 0) {
      return NextResponse.json(
        {
          error: `این نوع تعامل یا هدف در ${category.eventsCount} رویداد استفاده شده است و نمی‌توان آن را حذف کرد`,
        },
        { status: 400 }
      );
    }

    await IntentCategory.findByIdAndDelete(id);

    return NextResponse.json({
      success: true,
      message: 'نوع تعامل یا هدف با موفقیت حذف شد',
    });
  } catch (error) {
    console.error('❌ Error deleting intent category:', error);
    return NextResponse.json(
      {
        error: 'خطا در حذف نوع تعامل یا هدف',
        details: error.message,
      },
      { status: 500 }
    );
