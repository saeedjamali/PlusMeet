...
// GET - لیست دسته‌بندی تعامل اجتماعی
export async function GET(request) {
  try {
    ...
    const categories = await SocialDynamicsCategory.find(query)
    ...
    console.error("❌ Error fetching social dynamics categories:", error);
    return NextResponse.json(
      {
...
export async function POST(request) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: "شما دسترسی به ایجاد دسته‌بندی تعامل اجتماعی ندارید" },
        { status: 403 }
      );
    }
    ...
    const newCategory = new SocialDynamicsCategory(categoryData);
    ...
    return NextResponse.json(
      {
        success: true,
        message: "دسته‌بندی تعامل اجتماعی با موفقیت ایجاد شد",
        data: newCategory,
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("❌ Error creating social dynamics category:", error);
    ...
    return NextResponse.json(
      {
        error: "خطا در ایجاد دسته‌بندی تعامل اجتماعی",
        details: error.message,
      },
      { status: 500 }
    );
