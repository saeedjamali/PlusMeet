...
import SocialDynamicsCategory from "@/lib/models/SocialDynamicsCategory.model";
...
export async function GET(request, { params }) {
  try {
    ...
    const category = await SocialDynamicsCategory.findById(id)
    ...
    console.error("❌ Error fetching social dynamics category:", error);
    return NextResponse.json(
      {
        error: "خطا در دریافت دسته‌بندی تعامل اجتماعی",
...
export async function PUT(request, { params }) {
  try {
    ...
    const hasPermission = await checkPermission(
      authResult.user.id,
      "social_dynamics_categories",
      "update"
    );

    if (!hasPermission) {
      return NextResponse.json(
        { error: "شما دسترسی به ویرایش دسته‌بندی تعامل اجتماعی ندارید" },
        { status: 403 }
      );
    }
    ...
    if (body.code && body.code.toUpperCase().trim() !== category.code) {
      const existingCode = await SocialDynamicsCategory.findOne({
    ...
    return NextResponse.json({
      success: true,
      message: "دسته‌بندی تعامل اجتماعی با موفقیت به‌روزرسانی شد",
      data: category,
    });
  } catch (error) {
    console.error("❌ Error updating social dynamics category:", error);
    ...
    return NextResponse.json(
      {
        error: "خطا در به‌روزرسانی دسته‌بندی تعامل اجتماعی",
        details: error.message,
      },
      { status: 500 }
    );
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: "شما دسترسی به حذف دسته‌بندی تعامل اجتماعی ندارید" },
        { status: 403 }
      );
    }
    ...
    await SocialDynamicsCategory.findByIdAndDelete(id);

    return NextResponse.json({
      success: true,
      message: "دسته‌بندی تعامل اجتماعی با موفقیت حذف شد",
    });
  } catch (error) {
    console.error("❌ Error deleting social dynamics category:", error);
    return NextResponse.json(
      {
        error: "خطا در حذف دسته‌بندی تعامل اجتماعی",
        details: error.message,
      },
      { status: 500 }
    );
