...
export async function GET(request) {
  try {
    ...
    const categories = await FormatModeCategory.find(query)
    ...
    console.error("❌ Error fetching format/mode categories:", error);
    return NextResponse.json(
      {
...
export async function POST(request) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: "شما دسترسی به ایجاد نوع برگزاری ندارید" },
        { status: 403 }
      );
    }
    ...
    const newCategory = new FormatModeCategory(categoryData);
    ...
    return NextResponse.json(
      {
        success: true,
        message: "نوع برگزاری با موفقیت ایجاد شد",
        data: newCategory,
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("❌ Error creating format/mode category:", error);
    ...
    return NextResponse.json(
      {
        error: "خطا در ایجاد نوع برگزاری",
        details: error.message,
      },
      { status: 500 }
    );
